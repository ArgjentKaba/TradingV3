...
def test_governor_blocks_entry(monkeypatch):
    # Umgebung: alles erlaubt, außer Governor
    monkeypatch.setattr(app, "Governor", GovBlocks)
    ...
    bars = [
        ...
        _bar(t0 + timedelta(minutes=2), 1, 1, 1, 1),
    ]
    trades = app.backtest_variant(bars, "BTCUSDT", profile="SAFE", risk_perc=0.005)
    assert trades == [], "Governor sollte Entry blockieren."
