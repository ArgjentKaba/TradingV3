...
def _common_monkeypatch(monkeypatch):
    ...
    # Standardmäßig alles erlauben – pro Test überschreiben wir einzelne Gates
...
def test_no_trade_outside_session(monkeypatch):
    ...
    monkeypatch.setattr(app, "in_session", lambda *_a, **_k: False)
...
def test_gate_blocks_entry(monkeypatch):
    _common_monkeypatch(monkeypatch)
    monkeypatch.setattr(app, "passes_gate", lambda *_a, **_k: False)
    t0 = datetime(2025, 1, 1, 10, 0)
    bars = [_bar(t0, 1, 1, 1, 1), _bar(t0 + timedelta(minutes=1), 1, 1, 1, 1)]
    trades = app.backtest_variant(bars, "ETHUSDT", profile="SAFE", risk_perc=0.005)
    assert trades == []


def test_divergence_blocks_entry(monkeypatch):
    _common_monkeypatch(monkeypatch)
    monkeypatch.setattr(app, "divergence_ok", lambda *_a, **_k: False)
    t0 = datetime(2025, 1, 1, 11, 0)
    bars = [_bar(t0, 1, 1, 1, 1), _bar(t0 + timedelta(minutes=1), 1, 1, 1, 1)]
    trades = app.backtest_variant(bars, "SOLUSDT", profile="SAFE", risk_perc=0.005)
    assert trades == []
